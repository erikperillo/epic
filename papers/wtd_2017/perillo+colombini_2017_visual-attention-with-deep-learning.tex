...
-- either visual or from other senses-- and helps focus our cognitive processes
on what is important at a given moment.
We consider these facts to be a strong evidence that, in order to solve
...
region of an image for further cognitive processing~\cite{treisman_1980}.
...
\begin{center}
		\begin{tabular} {cc}
		\includegraphics[width=0.4\textwidth]{./img/giraffe_s.jpg} &
		\includegraphics[width=0.4\textwidth]{./img/giraffe_m.jpg}\\
        (a) & (b)
		\end{tabular}
\end{center}
...
\subsection{Computational models of Visual Saliency}
...
One example is \emph{Vocus}~\cite{frintrop_2005}, a model that extracts
features such as color contrast, orientation and luminance contrast from
different scales of the image to produce saliency maps like
...
for image classification such as \emph{VGG-16}~\cite{zisserman_2014}
...
We present two models, one using hand-crafted feature extraction
and the other using a fully convolutional neural network.
We then compare the results of both.

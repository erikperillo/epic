...
\begin{center}
		\begin{tabular} {cc}
		\includegraphics[width=0.33\textwidth]{./img/soccer_s.jpg} &
		\includegraphics[width=0.33\textwidth]{./img/soccer_m.jpg}\\
        (a) & (b)
		\end{tabular}
\end{center}
...
different scales of the image, features shown to be naturally salient to
humans~\cite{colombini_2016}.
...
\emph{MIT saliency benchmark}~\cite{mit_sal_bm}, a benchmark that uses
a variety of metrics to rank models, use \emph{Deep Learning} techniques.
...
\section{Methodology}
Implementations were made using \emph{Theano} version \texttt{0.9.0.dev}
...
\begin{figure}[hbt]
    ...
    \caption{Overview of the network.
        Filters sizes are in format w$\times$h\_stride.}
\end{figure}

Figure~\ref{fig:model} illustrates the overall architecture of our fully
convolutional neural network.
The network extracts features from increasingly smaller dimensions of the
input image. It is composed of four main blocks:
\begin{enumerate}
    ...
    \item Fourth and last level is composed of eight inception blocks
        ...
        the final saliency map with dimensions $W/8 \times H/8 \times 1$.
\end{enumerate}
...
Our prior tests showed slighly better performance using image-wise
...
\emph{SALICON}~\cite{salicon}, with 15000 images, and
...
The network was trained for 5 epochs with learning rate of $0.009$
and then for 3 epochs with learning rate of $0.001$.
We then switched to target normalized by unit normalization.
...
The network was trained for 2 epochs with learning rate of
...
\begin{table}[H]
    \centering
    \label{table:results}
    \caption{State of the art models and results on \tit{MIT300 benchmark}}
    \begin{tabular}{|c|c|c|c|c|c|c|}
        \hline
        ...
        \hline
        \emph{DeepFix} & $>$16.7 million & \textbf{0.87} & \textbf{0.78}
            & \textbf{2.26} & \textbf{0.67} & \textbf{2.04}\\
        \hline
        \emph{Salicon} & $>$14.7 million & \textbf{0.87} & 0.74 & 2.12
            & 0.60 & 2.62\\
        \hline
        \textbf{\emph{Ours}} & \textbf{3.72 million} & \textbf{0.85} &
        \textbf{0.71} & \textbf{1.98} & \textbf{0.62} & \textbf{2.37}\\
        \hline
        \emph{ML-Net} & $>$15.4 million & 0.85 & 0.69 & 2.07 & 0.60 & 2.53\\
        \hline
...
Figure~\ref{fig:preds} illustrate some predictions made by our model.
...
Metrics in bold show our values and the best values of the ranking.
Our model achieves results comparable to the best models, while having
at least one fourth of the parameters of any other model in the comparison.

\section{Conclusion}
In this paper, we proposed a novel fully convolutional network for the

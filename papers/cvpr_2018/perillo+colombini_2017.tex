...
\subsection{Training}
The loss function to be minimized was the
\emph{Correlation Coefficient} between the ground-truth saliency map $G$ and
the predicted map $P$:
$$CC(P, G) = \frac{cov(P, G)}{\sigma(P)\sigma(G)}$$
$CC$ is considered one of the most appropriate~\cite{judd_2016}
because it symmetrically penalizes both false positives and false negatives.
...
Proposed model achieved results comparable to those of the state of the art while having, at least, one-fourth of the number of parameters.
